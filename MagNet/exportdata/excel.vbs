...
'Begin calling code
'Currently using this as testing for all data export related functions
...
cur_time = exc_o.sprintf("{0:yyyyMMddhhmmss}", Array(now()))
...
Class export_data
  ...
  Private Function get_current_timestamp()
    get_current_time = file_name = sprintf("{0:yyyyMMddhhmmss}", Array(now()))
  End Function
  ...
  Public Function sprintf(sFmt, aData)
