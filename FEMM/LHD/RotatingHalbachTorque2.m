...
Aerr = [0 0 0 0 0 0 0 0 0];
Xerr = [-2 -2 -2 -2 0 2 2 2 2];

AngleSteps = 12;        % Change in angle over sweep angle
EnableBavg = false;
EnableBn = false;
EnableTorque = true;
...
for ai = 1:(AngleSteps+1)
    for X = 1:N
        ...
        XerrSum = 0;
        for i = 0:(N - 1)
            x = i*L + offset + Xerr(i + 1);
            theta = a + (ai-1)*SWEEP_ANGLE/AngleSteps + ra*i + Aerr(i + 1);
        ...
        if (EnableBavg && ai == 1 && X == 1)
            ...
            figure;
            plot(y, Bavg);
            xlabel('Height above magnets (mm)')
            ylabel('Bavg (T)')
        end
